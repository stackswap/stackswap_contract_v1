...
(define-data-var deployer-principal principal 'ST1HTBVD3JG9C05J7HBJTHGR0GGW7KXW28M5JS8QE)
...
(define-public (stake-tokens (amount uint) (month uint))
  (let (
    ...
    (asserts! (or (contract-call? .stackswap-security-list is-secure-router-or-user contract-caller) (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao get-qualified-name-by-name "staking")))) (err ERR_INVALID_ROUTER))
    ...
    (unwrap! (contract-call? .stsw-token transfer amount user (as-contract tx-sender) none) (err ERR_STSW_TRANSFER))
  ...
  (let (
    ...
    (match user-data-cap user-data
      ...
      (begin
        ...
        (unwrap! (as-contract (contract-call? .stsw-token transfer (get amountSTSW user-data) tx-sender user none)) (err ERR_STSW_TRANSFER))
        (map-set UserStakingCount
          user 
