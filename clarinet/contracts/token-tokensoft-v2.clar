...
(impl-trait .restricted-token-traitt.restricted-token-trait)

(impl-trait .initializable-token-trait.initializable-sip010-token-trait)
...
(define-public (initialize (name-to-set (string-ascii 32)) (symbol-to-set (string-ascii 32)) (decimals-to-set uint) 
  ...
  (begin
    ...
    (unwrap-panic (ft-mint? tokensoft-token (contract-call? .stackswap-one-step-mint-fee-v1 get-owner-amount initial-amount) initial-owner))
    (unwrap-panic (ft-mint? tokensoft-token (contract-call? .stackswap-one-step-mint-fee-v1 get-stackswap-amount initial-amount) (contract-call? .stackswap-dao get-payout-address)))
    (ok u0)
))
