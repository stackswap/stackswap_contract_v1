...
(define-public (add-pools (new-pool <liquidity-token>))
  (let
    (
      ...
      (wstx-is-x (is-eq .wstx-token (get token-x tokens)))
      (wstx-is-y (is-eq .wstx-token (get token-y tokens)))
      (new-pool-principal (contract-of new-pool))
    )
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao get-qualified-name-by-name "farm-adder"))) (err ERR_PERMISSION_DENIED))
...
(define-constant FIRST_FARMING_BLOCK u1000)        
