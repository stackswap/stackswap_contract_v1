...
(define-public (router-swap
  ...
  (let
    ...
    (if (and from-type true)
        (begin
            (try! (contract-call? .stackswap-swap-v1 swap-x-for-y from bridge from-lp from-amt from-2-bridge-min-amt))
        )
        (begin
            (try! (contract-call? .stackswap-swap-v1 swap-y-for-x bridge from from-lp from-amt from-2-bridge-min-amt))
        )
    )
    (let
        (
        ...
        (if (and to-type true)
            (begin
                (try! (contract-call? .stackswap-swap-v1 swap-x-for-y bridge to to-lp delta_bridge_amt bridge-2-to-min-amt))
            )
            (begin
                (try! (contract-call? .stackswap-swap-v1 swap-y-for-x to bridge to-lp delta_bridge_amt bridge-2-to-min-amt))
            )
        )
        (let
            (
