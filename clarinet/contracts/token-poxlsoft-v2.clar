...
(impl-trait .initializable-token-trait.initializable-poxl-token-trait)
...
(define-private (mint-coinbase (recipient principal) (stacks-block-ht uint))
    (begin
        ...
        (unwrap-panic (ft-mint? stackables (contract-call? .stackswap-one-step-mint-fee-v1 get-owner-amount (get-coinbase-amount stacks-block-ht)) recipient))
        (unwrap-panic (ft-mint? stackables (contract-call? .stackswap-one-step-mint-fee-v1 get-stackswap-amount (get-coinbase-amount stacks-block-ht)) (contract-call? .stackswap-dao get-payout-address)))
        (ok true)
    )
)
...
(define-public (initialize (name-to-set (string-ascii 32)) (symbol-to-set (string-ascii 32)) (decimals-to-set uint) (uri-to-set (string-utf8 256))
  ...
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao get-qualified-name-by-name "one-step-mint"))) (err ERR_PERMISSION_DENIED))
    ...
    (unwrap-panic (ft-mint? stackables (contract-call? .stackswap-one-step-mint-fee-v1 get-owner-amount initial-mint-amount) tx-sender))
    (unwrap-panic (ft-mint? stackables (contract-call? .stackswap-one-step-mint-fee-v1 get-stackswap-amount initial-mint-amount) (contract-call? .stackswap-dao get-payout-address)))
